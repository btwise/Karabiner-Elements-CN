...
static void hid_value_observer_callback(enum libkrbn_hid_value_type type,
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    if (simpleModificationJson.count > 0) {
      ...
      [queue updateAddSimpleModificationButton:[NSString stringWithFormat:@"将`%@`添加到Karabiner-Elements", name]];
    }
  });
}
...
- (void)pushMouseEvent:(NSEvent*)event {
  switch ([event type]) {
    ...
    case NSEventTypeOtherMouseDown:
      [self pushMouseEvent:event eventType:@"鼠标向下"];
      break;

    case NSEventTypeLeftMouseUp:
    case NSEventTypeRightMouseUp:
    case NSEventTypeOtherMouseUp:
      [self pushMouseEvent:event eventType:@"鼠标向上"];
      break;

    case NSEventTypeLeftMouseDragged:
    case NSEventTypeRightMouseDragged:
    case NSEventTypeOtherMouseDragged:
      [self pushMouseEvent:event eventType:@"鼠标拖动"];
      break;

    case NSEventTypeScrollWheel:
      [self pushScrollWheelEvent:event eventType:@"滚轮滚动"];
      break;
