...
public:
  ...
  void async_start(void) {
    enqueue_to_dispatcher([this] {
      if (client_manager_) {
        logger::get_logger().warn("grabber_client 已开始.");
        return;
      }
      ...
      client_manager_->connected.connect([this] {
        logger::get_logger().info("grabber_client 已连接.");
      ...
      client_manager_->closed.connect([this] {
        logger::get_logger().info("grabber_client 关闭了.");
      ...
      logger::get_logger().info("grabber_client 已开始.");
    });
  }
...
private:
  void stop(void) {
    ...
    logger::get_logger().info("grabber_client 已停止.");
  }
